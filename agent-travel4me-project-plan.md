...
- 每日运行依赖 agent 所在环境的定时能力，例如 cron、launchd、Windows Task Scheduler、GitHub Actions/self-hosted runner，或某个 agent 平台自己的 scheduled run。创建每日自动化是 durable trip 的默认动作；设置桌面壁纸才需要用户明确授权。
...
不同 coding agent 能力差异很大，skill 启动后必须先做环境探测，再决定走宿主原生工具、本地 provider，还是提示用户配置必要环境。每日定时任务在可用时直接创建，不把“是否开启自动化”作为用户问题。
...
- 有原生生图工具：优先用原生工具，并用第一次真实需要的图片生成验证能力，例如角色参考图或当天场景图；正式壁纸仍要检查它是否支持明确尺寸，不支持就本地 resize/crop。
...
6. 创建每日自动化；如果平台或系统要求安装授权，只在此时请求授权。
7. 生成第 1 天壁纸。
8. 询问是否允许设置为当前壁纸。
...
自动化任务不询问，创建后直接告知。自动换壁纸不要在首轮直接开启。建议在第一张正式壁纸生成后问：
...
面向用户的叙事默认只展示当天、已走过地点、总天数和大方向。完整 route 是内部状态；未来 waypoint、地标和完整路线表只在用户明确要求查看或导出时展示。Agent 不主动提供“是否继续生成”或“是否一口气画完剩余路线”的选项；用户明确要求时可以执行。
...
- 先告诉用户估算天数。
- 问是否要压缩旅程天数。
- 如果用户给出目标天数，例如“10 天内”，重新压缩 waypoint，保留主要地理转折和画面变化。
- 不问“旅行节奏”这类抽象配置。
...
`我算了一下，从深圳到里斯本，我大概需要 24 天能抵达。你想让我把旅程压缩一点吗？如果想，告诉我你希望几天内抵达，我会减少中间停留、让叙事节奏更紧凑。`
